# ---------------------------------------------------------------------------
# Changes applied (2025-08-23) by Leila Hernandez Rodriguez — Lawrence Berkeley National Laboratory
# - Fix return dict: use 'fr' instead of undefined 'frs'.
# - Add guards so fr/xrs/yrs always exist before return.
# - Clean 'is/== None' and other small comparisons in earlier patch.
# 
# ---------------------------------------------------------------------------
...
def FFP_climatology(zm=None, z0=None, umean=None, h=None, ol=None, sigmav=None, ustar=None,
                    ...
                    rs=[0.2,0.4,0.6,0.8,0.9], rslayer=0,
    ...
    # Handle rs
    if rs is not None:
        # Guards to ensure variables exist for return
        fr = locals().get('frs', locals().get('fr', None))
        xrs = locals().get('xrs', None)
        yrs = locals().get('yrs', None)
    ...
    if all(item is None for item in [dx, nx, domain]):
        ...
        domain = [-10000., 10000., -10000., 10000.] # By Leila, original was 1000
        dx = dy = 2.
        nx = ny = 10000 # By Leila
    ...
    if fig is None: fig = 0
    
    # --- safety defaults ---
    scale_const = 1.0   # ensure defined even if later branches are skipped
    ...
    for ix, (ustar, sigmav, h, ol, wind_dir, zm, z0, umean) \
        ...
        if not valids[ix]:
            ...
            if ol <= 0:   #convective
                ...
                # guard in case a refactor renamed or skipped the assignment
            if 'scale_const' not in locals() or scale_const is None:
                scale_const = 1.0
            if scale_const > 1:
                scale_const = 1.0
    ...
    if rs is not None:
        return {'x_2d': x_2d, 'y_2d': y_2d, 'fclim_2d': fclim_2d,
                'rs': rs, 'fr': fr, 'xr': xrs, 'yr': yrs, 'n':n, 'flag_err':flag_err}
    else:
        # Guards to ensure variables exist for return
        fr  = locals().get('fr',  locals().get('frs', None))
        xrs = locals().get('xrs', None)
        yrs = locals().get('yrs', None)
        return {'x_2d': x_2d, 'y_2d': y_2d, 'fclim_2d': fclim_2d,
...
def get_contour_vertices(x, y, f, lev):
    """
    Return [xr, yr] for the contour at level `lev`.
    Robust to empty contours and boundary-touching cases.

    Changes (2025-08-23):
    - Clamp `lev` inside data range to avoid "no levels found".
    - Handle empty `allsegs` safely (return [None, None]).
    - Choose the longest segment if multiple exist.
    - Return [None, None] if the contour touches the domain boundary.
    - Close the figure reliably.
    Requested by: Leila Hernandez Rodriguez — Lawrence Berkeley National Laboratory
    """
    import numpy as np
    import matplotlib.pyplot as plt

    # Guard: finite data?
    if f is None or not np.isfinite(np.asarray(f)).any():
        return [None, None]

    # Clamp level slightly within the scalar field range
    zmin = float(np.nanmin(f))
    zmax = float(np.nanmax(f))
    if not np.isfinite(lev):
        return [None, None]
    if zmax > zmin:
        eps = (zmax - zmin) * 1e-12
        lev = max(zmin + eps, min(zmax - eps, float(lev)))
    ...
    try:
        ...
        # No segments at this level -> no contour
        if not cs.allsegs or len(cs.allsegs[0]) == 0:
            return [None, None]

        # Pick the longest segment
        seg = max(cs.allsegs[0], key=lambda s: getattr(s, "shape", (0,))[0])
        seg = np.asarray(seg)
        if seg.size == 0:
            return [None, None]
        ...
        # Boundary guard: if the contour touches the domain limits, drop it
        # (domain from x/y which can be 1D or 2D)
        x_min = float(np.nanmin(x)); x_max = float(np.nanmax(x))
        y_min = float(np.nanmin(y)); y_max = float(np.nanmax(y))
        if (np.nanmin(seg[:, 0]) <= x_min) or (np.nanmax(seg[:, 0]) >= x_max) or \
           (np.nanmin(seg[:, 1]) <= y_min) or (np.nanmax(seg[:, 1]) >= y_max):
            return [None, None]
...
def plot_footprint(x_2d, y_2d, fs, clevs=None, show_heatmap=True, normalize=None, 
    ...
    # Added by Leila, Jan 2021
    # ---------------------------------------
    cmap = cm.hot
    cmap_reversed = cm.get_cmap('hot_r')
    # ---------------------------------------
    ...
    #if colormap is None: colormap = cm.jet     # Commented by Leila
    if colormap is None: colormap = cmap_reversed # Added by Leila 
    ...
    if clevs is not None:
        ...
        for f, c in zip(fs, cs):
            cc = [c]*len(levs)
            if show_heatmap:
                cp = ax.contour(x_2d, y_2d, f, levs, colors = 'k', linewidths=line_width)
            else:
                cp = ax.contour(x_2d, y_2d, f, levs, colors = 'k', linewidths=line_width)
            #Isopleth Labels
    ...
    if show_heatmap:
        ...
        #plt.title(aa)   # Added by Leila Jan 2021
        plt.xlabel('x [m]')
        plt.ylabel('y [m]')
        

        #Colorbar
        #cbar = fig.colorbar(im, shrink=1.0, format='%.3e') # Commented by Leila
        cbar = fig.colorbar(im, format='%.3e')
